(function(a, b, c) {
    ...
    do_related_post_start = function(json) {
        ...
        inject(def.homePage + '/feeds/posts/default' + la + '?alt=json-in-script&orderby=updated&start-index=' + ri + '&max-results=' + def.numPosts + '&callback=do_related_post_');
    },
...
if ("media$thumbnail" in entry[i] && def.thumbnailWidth !== false) {
img = entry[i].media$thumbnail.url.replace(/[^=]*[\d+]$|s\d+(\-c|-w\d+-h\d+-c)|s\d+(\-c)/,'w' + def.thumbnailWidth + '-' + 'h' + def.thumbnailHeight + '-n');
...
postauthor = entry[i].author[0].name.$t;

date = new Date(entry[i].published.$t);
...
if (month < 10) {
  month = '0' + month;
            ...
            if (st == 2) {
                sk += '<li><img alt="' + tt + '" class="' + ci + '-thumbnail" src="' + img + '" width="' + def.thumbnailWidth + '" height="' + def.thumbnailHeight + '"><a class="' + ci + '-title" title="' + tt + '" href="' + link + '"' + tg + '>' + title + '</a><span class="' + ci + '-summary"><span class="' + ci + '-summary-text">' + summary + '</span> <a href="' + link + '" class="' + ci + '-more"' + tg + '>' + def.moreText + '</a></span>' + cl + '</li>';
            } else if (st == 3 || st == 4) {
                sk += '<li class="' + ci + '" tabindex="0"><a class="' + ci + '-titleimg" href="' + link + '"' + tg + '><img alt="' + tt + '" class="' + ci + '-thumbnail" src="' + img + '" width="' + def.thumbnailWidth + '" height="' + def.thumbnailHeight + '"></a><div class="' + ci + '-tooltip"><a class="' + ci + '-title" title="' + tt + '" href="' + link + '"' + tg + '>' + title + '</a></div>' + '<div class="rpmeta"><div class="rpauthor">' + postauthor + '</div><div class="rpdate">' + dt + '/' + month + '/' + year + '</div></div>' + cl + '</li>';
            } else if (st == 5) {
                sk += '<li class="' + ci + '" tabindex="0"><a class="' + ci + '-wrapper" href="' + link + '" title="' + tt + '"' + tg + '><img alt="" class="' + ci + '-thumbnail" src="' + img + '" width="' + def.thumbnailWidth + '" height="' + def.thumbnailHeight + '"><span class="' + ci + '-tooltip">' + title + '</span></a>' + cl + '</li>';
            } else if (st == 6) {
                sk += '<li><a class="' + ci + '-title" title="' + tt + '" href="' + link + '"' + tg + '>' + title + '</a><div class="' + ci + '-tooltip"><img alt="" class="' + ci + '-thumbnail" src="' + img + '" width="' + def.thumbnailWidth + '" height="' + def.thumbnailHeight + '"><span class="' + ci + '-summary"><span class="' + ci + '-summary-text">' + summary + '</span></span>' + cl + '</div></li>';
            } else {
    ...
    a['do_related_post_'] = do_related_post;
    a['do_related_post_start_'] = do_related_post_start;
    // Generate random `start-index` data before loading the original related post data ...
    inject(def.homePage + '/feeds/posts/default?alt=json-in-script&orderby=updated&max-results=0&callback=do_related_post_start_');
